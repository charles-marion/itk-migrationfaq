...
class PMF_Faq
{
    ...
    */
     ...
     * @return integer
     */
    public function addErrorRecord(Array $data)
    {
      if(!isset($data['id_data'])||empty($data['id_data'])||!is_numeric($data['id_data']))
        {
        return false;
        }
      if($this->isErrorRecordByMd5($data['id_data'],md5($data['header'].$data['content'])))
        {
        return false;
        }
     ...
     * Returns an array with all data from a FAQ record
     *
     * @param  integer $id_data   data id
     * @return void
     */
    public function isErrorRecordByMd5($id_data,$md5)
    {
        $query =
            "SELECT
                *
            FROM
                faqerrors
            WHERE
                id_data=$id_data AND md5='$md5' limit 1 ";
